...
type ChannelOtherSettings struct {
	...
	OpenAIAggregator                      *OpenAIAggregatorConfig `json:"openai_aggregator,omitempty"`
}
...
type OpenAIAggregatorConfig struct {
	Upstreams []OpenAIAggregatorUpstream `json:"upstreams,omitempty"`
}

type OpenAIAggregatorUpstream struct {
	BaseURL string `json:"base_url,omitempty"`
	Key     string `json:"key,omitempty"`
}

func (c *OpenAIAggregatorConfig) Validate() error {
	if c == nil {
		return fmt.Errorf("openai_aggregator is required")
	}
	if len(c.Upstreams) == 0 {
		return fmt.Errorf("openai_aggregator.upstreams requires at least one upstream")
	}
	for i, upstream := range c.Upstreams {
		baseURL := strings.TrimSpace(upstream.BaseURL)
		if baseURL == "" {
			return fmt.Errorf("openai_aggregator.upstreams[%d].base_url is required", i)
		}
		parsedURL, err := url.Parse(baseURL)
		if err != nil || parsedURL.Scheme == "" || parsedURL.Host == "" {
			return fmt.Errorf("openai_aggregator.upstreams[%d].base_url must be a full URL", i)
		}
		if !strings.EqualFold(parsedURL.Scheme, "http") && !strings.EqualFold(parsedURL.Scheme, "https") {
			return fmt.Errorf("openai_aggregator.upstreams[%d].base_url must use http or https", i)
		}
		if strings.TrimSpace(upstream.Key) == "" {
			return fmt.Errorf("openai_aggregator.upstreams[%d].key is required", i)
		}
	}
	return nil
}
